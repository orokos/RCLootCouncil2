...
do
   -- create the other buttons/responses
	for i = 1, addon.Const.DEFAULT_OPTIONS.profile.maxButtons do
		if i > addon.Const.DEFAULT_OPTIONS.profile.buttons.default.numButtons then
			tinsert(addon.Const.DEFAULT_OPTIONS.profile.buttons.default, {
				text = L["Button"].." "..i,
				whisperKey = ""..i,
			})
			tinsert(addon.Const.DEFAULT_OPTIONS.profile.responses.default, {
				color = {0.7, 0.7,0.7,1},
				sort = i,
				text = L["Button"]..i,
			})
	...
	for i = #addon.Const.DEFAULT_OPTIONS.profile.awardReasons+1, addon.Const.DEFAULT_OPTIONS.profile.maxAwardReasons do
		tinsert(addon.Const.DEFAULT_OPTIONS.profile.awardReasons, {color = {1, 1, 1, 1}, disenchant = false, log = true, sort = 400+i, text = "Reason "..i,})
	end
end
